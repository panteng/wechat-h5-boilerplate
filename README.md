...
## 项目结构
        ...
        /app
            /dist               --> 项目文件的分发版本，所有的文件均由webpack生成，请勿手动修改
                /media        --> 从app/src/audios复制而来
                /fonts          --> 从app/src/fonts和在vendors中指定的第三方字体复制而来
                /img         --> 由app/src/images下的图片经Imagemin压缩优化生成
                /js    --> 由app/src/javascripts下的文件经Browserify打包生成
                /css    --> 由app/src/scss下的文件编译生成
                index.html      --> 由app/src/index.html经Gulp-inject插入bundle.(min.).css和bundle.(min.).js后生成
            /src                --> 项目的源码，所有文件都可编辑
                /audios         --> 存放音频、视频文件
                /fonts          --> 存放字体文件
                /images         --> 存放图片文件
                /js    --> JS源文件，经Browserify打包后生成app/dist/js
                /scss           --> SCSS文件，经过编译后生成app/dist/css
...
1. **将本项目clone到本地**
    ...
    在控制台中运行：
        ...
        git clone --depth=1 https://github.com/panhezeng/wechat-h5-boilerplate.git <your-project-name>
        cd <your-project-name>
        
    或者你也可以直接在[Release](https://github.com/panhezeng/wechat-h5-boilerplate/releases)页面下载WHB的源码压缩包。
...
7. **移动端调试**
    ...
    首先，运行`npm run dev`任务，在控制台的输出信息中找到下面这段：
